...
const nextConfig = {
  ...
  // SPIKE, non da mergere cosi' com'e'. Domanda: la CDN di Vercel accetta di cachare
  // le risposte di pagine rese dinamicamente? Oggi ogni pagina esce con
  // `cache-control: private, no-store` e `x-vercel-cache: MISS` perche' il root layout
  // legge headers(). Se questi due header bastano, si ottiene il beneficio (cache CDN,
  // TTFB, crawl budget) senza spostare 158 file in route group.
  //
  // CDN-Cache-Control e Vercel-CDN-Cache-Control sono letti dalla CDN e NON vengono
  // mostrati al browser, quindi non toccano il comportamento del client: e' per questo
  // che possono convivere con il no-store che Next emette dalla lambda.
  //
