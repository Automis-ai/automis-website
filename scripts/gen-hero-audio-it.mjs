// Generate the hero "live call" audio in ITALIAN (it-IT) via ElevenLabs,
// plus a synced caption cue file. API key read from env; NEVER written to a shipped file.
//
// Usage:
//   source ~/.claude/secrets/api-keys.env
//   node scripts/gen-hero-audio-it.mjs voices        # list account voices
//   node scripts/gen-hero-audio-it.mjs               # generate (uses it-IT voices below)
//
// Output (both shipped; the key is not):
//   public/audio/hero-call-it.wav
//   public/audio/hero-call-it.cues.json   [{ start, end, speaker, name, text }]
...
const OUT_WAV = join(ROOT, "public", "audio", "hero-call-it.wav");
const OUT_CUES = join(ROOT, "public", "audio", "hero-call-it.cues.json");
const H = { "xi-api-key": KEY };
const SR = 22050; // mono 16-bit WAV — universally supported & seekable; small enough for voice

// ITALIAN (it-IT) voices — override via env if desired (AGENT_VOICE / CALLER_VOICE).
const AGENT = process.env.AGENT_VOICE || "QITiGyM4owEZrBEf0QV8"; // Ginevra (f, standard it-IT)
const CALLER = process.env.CALLER_VOICE || "G1QO6RfZl0zS1DpKDReq"; // Ricasco – Warm & Friendly (m, it-IT)
const MODEL = "eleven_multilingual_v2";

// The call (Italian). Agent answers first (inbound).
const LINES = [
  { who: "agent", name: "Automis", text: "Automis, buonasera. Come posso aiutarla?" },
  { who: "caller", name: "Cliente", text: "Buonasera. Vorrei prenotare un appuntamento." },
  { who: "agent", name: "Automis", text: "Certamente. Ho un posto giovedì, alle dieci e mezza. Le va bene?" },
  { who: "caller", name: "Cliente", text: "Sì, perfetto." },
  { who: "agent", name: "Automis", text: "Benissimo. Le ho appena inviato un SMS con la conferma. Desidera altro?" },
  { who: "caller", name: "Cliente", text: "No, grazie mille." },
  { who: "agent", name: "Automis", text: "Di nulla. Le auguro una buona serata!" },
];
...
function mp3ToPcm(buf, idx) {
  const mp3 = join(tmpdir(), `av-it-seg-${idx}.mp3`);
  const wav = join(tmpdir(), `av-it-seg-${idx}.wav`);
