...
const HeroSection = () => {
  ...
  useEffect(() => {
    let alive = true;
    fetch("/audio/hero-call-it.cues.json")
      .then((r) => (r.ok ? r.json() : []))
      .then((d) => {
  ...
  const buildEnvelope = async () => {
    ...
    try {
      const res = await fetch("/audio/hero-call-it.wav");
  ...
  useEffect(() => {
    ...
    const onError = () => {
      ...
      const map = { 1: "interrotto", 2: "rete", 3: "decodifica", 4: "formato non supportato" };
      setAudioMsg(`Errore audio: ${map[code] || code || "sconosciuto"}`);
    };
  ...
  const togglePlay = async () => {
    ...
    if (audio.paused) {
      ...
      try {
        ...
        setAudioMsg(`Impossibile riprodurre: ${e?.name || ""} ${e?.message || ""}`.trim());
      }
    } else {
      audio.pause();
    }
        ...
        <div className="grid items-center gap-12 lg:grid-cols-[1.05fr_0.95fr] lg:gap-16">
          ...
          <div className="text-center lg:text-left">
            ...
            >
              ...
              artificiale vocale gestisce i contatti, qualifica i clienti e fissa
              gli appuntamenti — mentre tu ti concentri sulla crescita del tuo
              business.
            </p>
          ...
          >
            <div className="av-gradient-border rounded-[28px] p-6 shadow-2xl shadow-black/40 backdrop-blur-xl sm:p-7">
              <audio ref={audioRef} src="/audio/hero-call-it.wav" preload="metadata" />
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="leading-tight">
                    ...
                    <p className="font-plex-mono text-[11px] uppercase tracking-wider text-soft-blue/70">
                      {playing ? "In chiamata" : "Dimostrazione"}
                    </p>
                  </div>
                </div>
                ...
                >
                  {playing && cue ? (
                    ...
                    <p className="font-open-sans text-sm italic leading-snug text-white/70">
                      {playing
                        ? "In collegamento…"
                        : "Ascolta una vera chiamata, in italiano."}
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="flex items-center justify-center gap-5">
                ...
                <button
                  type="button"
                  onClick={togglePlay}
                  aria-label={playing ? "Metti in pausa la chiamata" : "Ascolta la chiamata"}
              ...
              <p className="mt-3 text-center font-plex-mono text-[11px] uppercase tracking-wider text-soft-blue/60">
                {playing ? "Riproduzione della chiamata" : "Premi ▶ per ascoltare"}
              </p>
              {audioMsg && (
