...
const HeroSection = () => {
  ...
  useEffect(() => {
    let alive = true;
    fetch("/audio/hero-call-en.cues.json")
      .then((r) => (r.ok ? r.json() : []))
      .then((d) => {
  ...
  const buildEnvelope = async () => {
    ...
    try {
      const res = await fetch("/audio/hero-call-en.wav");
  ...
  useEffect(() => {
    ...
    const onError = () => {
      ...
      const map = { 1: "aborted", 2: "network", 3: "decode", 4: "unsupported format" };
      setAudioMsg(`Audio error: ${map[code] || code || "unknown"}`);
    };
  ...
  const togglePlay = async () => {
    ...
    if (audio.paused) {
      ...
      try {
        ...
        setAudioMsg(`Unable to play: ${e?.name || ""} ${e?.message || ""}`.trim());
      }
    } else {
      audio.pause();
    }
        ...
        <div className="grid items-center gap-12 lg:grid-cols-[1.05fr_0.95fr] lg:gap-16">
          ...
          <div className="text-center lg:text-left">
            ...
            <p
              ...
              data-aos-duration={1100}
            >
              Turn every call into an opportunity. Our voice AI handles your
              contacts, qualifies customers and books appointments — while you
            ...
            >
              {[
                { icon: Clock, label: "Up and running in 72 hours" },
                { icon: ShieldCheck, label: "GDPR compliant" },
                { icon: Globe, label: "Natural English voice" },
              ].map(({ icon: Icon, label }) => (
                <li
                  key={label}
          ...
          >
            <div className="av-gradient-border rounded-[28px] p-6 shadow-2xl shadow-black/40 backdrop-blur-xl sm:p-7">
              <audio ref={audioRef} src="/audio/hero-call-en.wav" preload="metadata" />
                ...
                >
                  {playing && cue ? (
                    ...
                    <p className="font-open-sans text-sm italic leading-snug text-white/70">
                      {playing
                        ? "Connecting…"
                        : "Listen to a real call, in English."}
                    </p>
                  )}
                </div>
              </div>
              ...
              <p className="mt-3 text-center font-plex-mono text-[11px] uppercase tracking-wider text-soft-blue/60">
                {playing ? "Playing the call" : "Press ▶ to listen"}
              </p>
              {audioMsg && (
