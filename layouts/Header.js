...
const DefaultHeader = ({ onePage }) => {
  ...
  return (
    <>
    <header
      className={`container main-header menu-absolute header-white no-border flex items-center fixed-header ${isSticky ? 'scrolled' : 'transparent-header'}`}
      style={{
        position: 'fixed',
    ...
    >
      <div className="header-upper" style={{
...
const MobileMenuContent = ({ menus, onePage, activeLink, setActiveLink, setMobileMenuOpen }) => {
          ...
          {onePage ? (
                  ...
                  {menu.submenus && (
                    <>
                      <ul className="dropdown-menu" style={{
                        ...
                        transform: activeDropdown === menu.id ? 'translateY(8px)' : 'translateY(0)',
                        transition: 'all 0.25s ease',
                        zIndex: '1000'
                      }}>
                        {menu.submenus.map((submenu) => (
                          <li key={submenu.id}>
                            <a href={`#${submenu.href}`} style={{ color: 'white', padding: '8px 20px', display: 'block' }}>{submenu.title}</a>
                          </li>
                        ))}
                      </ul>
                      <div className="dropdown-btn" style={{
            ...
            <ul className="navigation clearfix">
              {menus.map((menu) => (
                ...
                >
                  <Link
                    href={menu.submenus ? '#' : (menu.href === 'home' ? '/' : `/${menu.href}`)}
                    onClick={(e) => {
                      if (!menu.submenus) {
                        setActiveLink(menu.href);
                      } else {
                    ...
                    onMouseEnter={(e) => { if (activeLink !== menu.href && !menu.submenus) e.target.style.color = '#3C91E6'; }}
                    onMouseLeave={(e) => { if (activeLink !== menu.href && !menu.submenus) e.target.style.color = ''; }}
                  >
                    {menu.title}
                    {menu.submenus && (
                      <span
                          ...
                          <li key={submenu.id}>
                            <Link
                              ...
                              onMouseEnter={(e) => {
                                ...
                                e.target.style.textShadow = '0 0 12px rgba(60, 145, 230, 0.8), 0 0 20px rgba(60, 145, 230, 0.4)';
