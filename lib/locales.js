...
export function resolveTarget(pathname, targetLang) {
  // The Portuguese site is served at /pt/* by a middleware rewrite to /pt-site/*.
  // usePathname() reports the public path in production, but the internal route is
  // reachable directly too (automis.ai/pt-site/blog answers 200, canonical'd to
  // /pt/blog), and there the raw path would produce "/it/pt-site/blog" - a link to
  // a 404, published in the HTML. Normalise before doing anything else.
  const publicPath = (pathname || "/").replace(/^\/pt-site(?=\/|$)/, "/pt");

  const noLang = stripLangPrefix(publicPath);
