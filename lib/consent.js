...
// Internal team self-exclusion. A browser flagged via ?internal=1 is never
// tracked, across GA4 / Clarity / Meta at once, independent of IP (IPs are
// dynamic / behind VPNs, so IP filters are unreliable). ?internal=0 clears it.
export const INTERNAL_FLAG = "automis_internal";
...
export function applyConsent(categories) {
  ...
  if (isInternalTraffic()) {
    // Internal team browser: keep everything denied no matter what was clicked,
    // and never emit the cookie_consent_update that would fire the tags.
    if (typeof window.gtag === "function") {
      window.gtag("consent", "update", toConsentSignals(DENY_ALL));
    }
    return;
  }
  ...
  /*
    The GTM container does not trigger GA4, Clarity and the Meta pixel on page
    view: it triggers them on the custom event `cookie_consent_update`, which
    CookieYes used to push. With CookieYes gone we must emit it ourselves, or
    those tags never fire at all, even for a visitor who accepted.

    Firing it after a rejection is safe: each of those tags carries a GTM
    consent check (analytics_storage / ad_storage), so a denied signal still
    blocks them. Emit it *after* the consent update above, never before.

    Once the container's triggers move to All Pages + consent checks, this
    push can go away.
  */
